...
function Completed ({ data }) {
  return (
    <div className={styles.sales}>
        <div className={styles.title}>往年已处理未处理的报警事件总数</div>
        <ResponsiveContainer minHeight={360}>
            <AreaChart data={data}>
                ...
                />
                <Area type="monotone" dataKey="已处理" stroke={color.grass} fill={color.grass} strokeWidth={2} dot={{ fill: '#fff' }} activeDot={{ r: 5, fill: '#fff', stroke: color.green }} />
                <Area type="monotone" dataKey="未处理" stroke={color.sky} fill={color.sky} strokeWidth={2} dot={{ fill: '#fff' }} activeDot={{ r: 5, fill: '#fff', stroke: color.blue }} />
            </AreaChart>
        </ResponsiveContainer>
    </div>
  )
